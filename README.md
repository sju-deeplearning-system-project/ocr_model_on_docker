# <세종대학교 딥러닝시스템 기말 프로젝트>
## 관광 음식메뉴판 데이터
설명: AI허브에서 제공하는 "관광 음식메뉴판 데이터"의 AI 모델코드의 도커실행 시 파일입니다.  
도커파일은 따로 저장하지 않았으므로 원본 AI모델에서 주어지는 이미지파일을 사용하면 됩니다.

## 도커 이미지 환경 구축
* 도커 파일이 있다는 가정하의 가이드입니다.
### 도커 이미지 다운로드
```bash
docker load -i ko-menu-text-recognition
```
### 도커 실행
이 레파지토리에는 도커을 구축했을 경우의 파일들이고 그 전에 환경 구축하는 방법을 적을 겁니다.  

  ```bash
docker run -it --shm-size=8GB ko-menu-text-recognition:latest /bin/bash
```
우선 도커를 실행합니다.  
여기서 중요한 사항은 공유 메모리를 반드시 8GB로 설정해야 합니다.  
그렇지 않으면 학습도중 프로세스가 죽어버리는 현상이 발생합니다.  
만약 --shm-size=8GB로도 충분하지 않은 경우 임의로 값을 올려주면 됩니다. 
### OCR 학습 모델 적용 (샘플데이터)
#### 기존 명령어
```bash
python inferences/text_recognition.py --Transformation TPS --FeatureExtraction VGG --SequenceModeling BiLSTM --Prediction Attn --image_folder data/data/testset_129104 --saved_model saved_models/TPS-VGG-BiLSTM-Attn-final/best_accuracy.pth --batch_max_length 50 --workers 16 --batch_size 128 --imgH 64 --imgW 300 
```
여기서도 참고해야할 부분이 있습니다.  
기존에 제시되는 컴퓨터 스펙과는 차이가 있을 수 있으므로 저는 배치사이즈를 64로 줄여서 학습을 진행했습니다.  
만약 사양이 낮은 경우 프로세스가 죽을 수도 있기 때문에 배치사이즈를 낮춰주시길 권장드립니다.
#### 저사양 전용 명령어
```bash
python inferences/text_recognition.py --Transformation TPS --FeatureExtraction VGG --SequenceModeling BiLSTM --Prediction Attn --image_folder data/data/testset_129104 --saved_model saved_models/TPS-VGG-BiLSTM-Attn-final/best_accuracy.pth --batch_max_length 50 --workers 16 --batch_size 64 --imgH 64 --imgW 300 
```

### 결과 경로
```bash
/data/result/2-084_text-recognition_result_0401153329.xlsx
```

허브데이터 출처: https://www.aihub.or.kr/aihubdata/data/view.do?currMenu=115&topMenu=100&aihubDataSe=data&dataSetSn=71553  
도커 가이드 출처: https://twigfarm.notion.site/2-084-e2845beb0f8b4ecf9d54d446f3b570ca
